...
//!
//!   X := pointer to the top left corner of the image, aligned to 32 bytes boundary
...
YuvComponent::YuvComponent(const uint width, const uint height) {
  //
  if (enable_trace) {
    std::cout << "TRACE:: allocating a component: " << width << "x" << height  << std::endl;
  }
...
void YuvComponent::write_to_stream(std::ofstream& outstrm) const {
  if (enable_trace) {
    std::cout << "TRACE:: component write to stream: " << width_ << "x" << height_  << std::endl;
  }
...
void YuvComponent::downsize_from(const YuvComponent &src, Decimation *poly_ptr) {
  //
  //
  YuvComponent tmp(width_, src.height_);

  poly_ptr->do_horizontal(
      tmp.ptr_, src.ptr_,
      width_, src.height_,
      tmp.stride_, src.stride_);
  poly_ptr->do_vertical(
      ptr_, tmp.ptr_,
      width_, height_,
      stride_, tmp.stride_);
}
...
//! Not yet implemented => calls the standard version

void YuvComponent::downsize_from(const YuvComponent &src, const uint max_thread, Decimation *poly_ptr) {
  //
  //
  downsize_from(src, poly_ptr);
}
...
//! Non threaded
//
void YuvImage::downsize_from(const YuvImage &src_image, const uint max_thread, Decimation *poly_ptr) {
  for (uint i = 0; i < components_.size(); ++i) {
    components_[i].downsize_from(src_image.components_[i], max_thread, poly_ptr);
  }
}
...
//!
void YuvImage::downsize_threaded_by_component(const YuvImage &src_image, Decimation *poly_ptr) {
  std::thread t_y([&]() {
    components_[0].downsize_from(src_image.components_[0], poly_ptr);
  });
  std::thread t_u([&]() {
    components_[1].downsize_from(src_image.components_[1], poly_ptr);
  });

  components_[2].downsize_from(src_image.components_[2], poly_ptr);
...
YuvImage::YuvImage(const std::string &file_name, const uint width, const uint height) {

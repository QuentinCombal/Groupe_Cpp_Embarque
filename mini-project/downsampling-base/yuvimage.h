...
class YuvComponent {
 ...
 public:
  ...
  void downsize_from(const YuvComponent &src, Decimation *poly_ptr);
  void downsize_from(const YuvComponent &src, const uint max_thread, Decimation *poly_ptr);
};


class YuvImage {
 public:
  explicit YuvImage(const std::string &file_name, const uint width = 0, const uint height = 0);
  explicit YuvImage(const uint width, const uint height);
  YuvImage(const YuvImage &image);
  ~YuvImage() = default;
  void downsize_from(const YuvImage &src_image, const uint max_thread, Decimation *poly_ptr);
  void downsize_threaded_by_component(const YuvImage &src_image, Decimation *poly_ptr);

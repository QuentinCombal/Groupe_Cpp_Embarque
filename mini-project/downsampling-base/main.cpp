...
void TopConfig::main() {
  ...
  Decimation *poly_ptr;
  if (method == "simple"){
      poly_ptr = new SimpleDecimation(max_thread_);
  } else if (method == "convolution"){
    poly_ptr = new ConvolutionDecimation(max_thread_);
  } else {
    poly_ptr = new VectorizedDecimation(max_thread_);
  }
  //  measure the time and start downsizing loop
  auto time_start = std::chrono::high_resolution_clock::now();
  for (int i = 0; i < 10; ++i) {
    result_image.downsize_from(input_image, max_thread_, poly_ptr);
  }
  ...
  std::cout << "Info: downsized image stored in file " << output_file_name_ << std::endl;
}
  ...
  if (argc > 1) {
    ...
    do {
      ...
      if (current_arg == "-h" || current_arg == "--help") {
        ...
        idx++;
        if (idx >= argc) {
          std::cerr << "Error: argument " << current_arg << " needs a parameter" << std::endl;
          exit(-1);
        }
        std::string integer_arg(argv[idx]);
        if (!is_integer(integer_arg)) {
          std::cerr << "Error: argument " << integer_arg << " is not an integer" << std::endl;
          exit(-1);
        }
      ...
      // String arguments
        ...
        std::string string_arg(argv[idx]);
        if ((string_arg == "simple") ||
           (string_arg == "convolution") ||
           (string_arg == "vectorized")){
          method = string_arg; 
      } else {
        std::cerr << "Error: argument " << string_arg << " is not a valid method" << std::endl;
      }
      } else if (current_arg == "--output") {
        output_file_name_ = current_arg;
      } else if (file_name_ != "") {
         std::cerr << "Error: argument " << current_arg << " is not valid" << std::endl;
         exit(-1);
      } else {
